...
| `mana_barrier` | magic | blocksVision:[astral] | — | magical |
...
Alias legacy acceptés (auto-résolus) : `turret`→automatic_turret, `barrier`→mana_barrier, `guard`→armed_guard.
...
- **structural** : `wall` 3×0.35 M V · `pillar` 1×1 M V · `opaque_door` 1×0.35 M V · `opening` 1.5×0.25 (floor, passage dans un mur) · `glass` 2×0.2 M (vitre : bloque mouvement, PAS la vision) · `grid` 2×0.2 M (grille : idem)
