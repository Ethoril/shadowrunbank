...
const EntityCatalog = (() => {
    ...
    const types = {
        steel_grate: define({ category: 'structure', name: "Grille d'acier", label: 'GRL', icon: 'steel-grate', color: '#78909c', blocksMovement: true, stateProfile: 'structural' }),

        mad_gate: define({ category: 'access', name: 'Portique MAD', label: 'MAD', icon: 'mad-gate', color: '#00bcd4', networkable: true, coverageType: 'threshold', coverageChannel: 'magnetic', defaultCoverage: { range: 0.75, width: 2 }, accessControl: true, stateProfile: 'access' }),
        maglock: define({ category: 'access', name: 'Maglock', label: 'MAG', icon: 'maglock', color: '#00d2ff', networkable: true, accessControl: true, stateProfile: 'access' }),
        retina_scanner: define({ category: 'access', name: 'Scanner rétinien', label: 'RET', icon: 'retina-scanner', color: '#29b6f6', networkable: true, accessControl: true, biometric: true, stateProfile: 'access' }),
        dna_analyzer: define({ category: 'access', name: 'Analyseur ADN', label: 'ADN', icon: 'dna-analyzer', color: '#26c6da', networkable: true, accessControl: true, biometric: true, stateProfile: 'access' }),
        elevator: define({ category: 'access', name: 'Ascenseur', label: 'ELV', icon: 'elevator', color: '#8899ff', networkable: true, accessControl: true, stateProfile: 'access' }),

        camera: define({ category: 'detection', name: 'Caméra', label: 'CAM', icon: 'camera', color: '#ff2a2a', networkable: true, coverageType: 'cone', coverageChannel: 'optical', defaultCoverage: { angle: 60, range: 6 }, canSweep: true }),
        ...
        combat_drone: define({ category: 'defense', name: 'Drone de combat', label: 'DRN', icon: 'combat-drone', color: '#ff2a9d', networkable: true, canPatrol: true, coverageType: 'cone', coverageChannel: 'optical', defaultCoverage: { angle: 80, range: 7 }, canSweep: true, armed: true, stateProfile: 'drone' }),
        ...
        network_node: define({ category: 'utility', name: 'Nœud réseau', label: 'NET', icon: 'network-node', color: '#ffb300' })
    };
...
const DecorCatalog = (() => {
    ...
    const types = {
        ...
        safe: define({ category: 'furniture', name: 'Coffre-fort', label: 'COF', icon: 'safe', color: '#546e7a', width: 1.5, height: 1.5, blocksMovement: true, blocksVision: opaque, autoDiscover: false }),
