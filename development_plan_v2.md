...
| 2026-07-19 | Utilisation joueur | Refonte tablette : le choix de destination d'une transition passait par `prompt()`, inutilisable sur tablette (le « 1 » prérempli n'y est pas présélectionné : taper « 2 » donnait « 12 » → destination invalide, échec silencieux — « rien ne se passe »). Nouvelle modale tactile (`transition-dialog.js`) : destinations en gros boutons (ordre des étages, noms réels pour les ascenseurs, lettres a/b/c des points), et embarquement groupé — les PJ à ≤ 0,8 case du point (dans la cabine) sont cochés d'office, ceux à ≤ 1,5 case proposés décochés. Nouveau `Exploration.moveGroupThroughTransition` : arrivée en couronne (offsets 0,7) sans empilement, bornée à la grille. Échec (passage verrouillé/hors ligne) désormais affiché dans la modale au lieu d'un silence. Boutons activés au `pointerup` tactile (l'émulation CDP ne synthétise pas de `click` après un tap) avec garde anti-double. Repli confirm()/prompt() conservé hors DOM. Tests : test tactile réécrit sur la modale, nouveau test d'intégration ascenseur multi-PJ, test unitaire du déplacement groupé (36/36 et 19/19 verts) | a0eeb01 |
| 2026-07-19 | Découverte automatique | Correctif : emprunter une liaison (ou voir une cabine d'ascenseur) enregistrait la découverte au niveau transition, révélant d'un coup TOUTES ses sorties sur tous les étages — même jamais visitées. La découverte passe au niveau point de passage (`kind: 'endpoint'`) : l'usage révèle les deux points traversés, la vue d'une cabine ne révèle que l'arrêt de son étage. Migration des anciens enregistrements vers les points de l'étage où la découverte a eu lieu (les autres sorties redeviennent cachées), poussée au cloud côté MJ. Et l'œil (arbre de visibilité) comme le toggle inspecteur reflètent désormais la visibilité EFFECTIVE (révélé MJ ou découvert) : un point découvert par les pions n'apparaît plus « caché » côté MJ, et « cacher » retire aussi la découverte (nouveau `Store.removeDiscovery`). Coches d'embarquement de la modale : tous les PJ de l'étage sont proposés (triés par distance, cochés d'office sur le point) au lieu du seul rayon de 1,5 case, trop strict en pratique. Tests 38/38 et 19/19 verts | f934af2 |
| 2026-07-19 | Découverte automatique | Correctif (suite de f934af2) : pendant qu'un écran non rechargé tourne encore l'ancienne version (onglet ouvert, cache index ≤ 10 min), ses découvertes s'écrivent à l'ancien format « transition » ; l'écran MJ à jour les migrait en les SUPPRIMANT du cloud immédiatement — la découverte « disparaissait instantanément » sous les yeux du joueur, en boucle. La migration devient non destructive pendant la transition : un enregistrement ancien format récent (< 6 h) est converti localement et éclaté en points, mais laissé au cloud pour l'écran qui l'a écrit ; seuls les enregistrements plus vieux que la période de grâce sont purgés. Test unitaire du comportement (récent conservé, ancien purgé). Tests 39/39 et 19/19 verts | cfff22b |
| 2026-07-19 | Découverte automatique | Correctif (suite de d972292) : l'élargissement trans-mur des décors fins s'appliquait à TOUT décor plus fin qu'une case — une étagère (mobilier, 2 × 0,6) plaquée contre un mur devenait visible depuis la pièce voisine. L'élargissement est désormais réservé aux décors de catégorie `structural` (mur, porte, ouverture, vitre, grille), qui vivent DANS la cloison ; le mobilier fin (étagère, armoire, plante) garde son empreinte réelle et n'est révélé que depuis sa propre pièce. Test unitaire de régression (étagère invisible depuis la pièce d'à côté, visible depuis la sienne). Tests 40/40 et 19/19 verts | b1e15dd |
| 2026-07-19 | Confort d'édition | L'inspecteur d'un nœud réseau liste désormais les appareils qui lui sont reliés (tous étages confondus, triés par étage puis par nom), miroir de la liaison `networkId` déjà saisie côté appareil. Chaque ligne affiche le nom et l'étage de l'appareil (type en info-bulle), un bouton → qui le sélectionne (changement d'étage si besoin, recadrage et pulse comme depuis l'arbre de visibilité) et un bouton × qui le délie du nœud. Tests 40/40 et 19/19 verts | a696e00 |
| 2026-07-21 | Performance | Fluidité tablette, trois optimisations sans changement visuel. (1) Occulteurs : la validité du cache passe d'une signature `JSON.stringify` de tout l'étage recalculée à CHAQUE appel (une fois par entité à couverture, à 30 fps) à un jeton bon marché = compteur de mutations du plan (`Store.getMutationSeq`, incrémenté dans `markPlanDirty`) + une époque locale (`invalidateOccluders`, bumpée au rendu et lors d'un glisser d'occulteur). La géométrie ne changeant qu'à l'édition, le cache reste valide pendant l'animation → plus aucune sérialisation par frame. (2) Boucle `requestAnimationFrame` : au lieu de balayer toutes les entités de l'étage à 60 fps (avec un `find` réseau O(N) chacune) même à l'arrêt, `Anim.refresh` (appelé à chaque rendu) met en cache la seule liste des entités animées (ronde en mouvement ou balayage) ; la boucle n'itère que celle-ci et s'arrête quand elle est vide (0 CPU au repos). (3) Overlay SVG : `renderCoverages` et `renderCables` (appelés à chaque frame) réconcilient les enfants du groupe par clé (`reconcileSvgGroup`) — nœuds réutilisés + attributs mis à jour au lieu de `innerHTML=''` puis recréation, supprimant le churn DOM/GC. Tests 40/40 unitaires + smoke occulteurs/overlay (nœud SVG réutilisé, cache occulteurs 1 build pour 50 requêtes) ; intégration Playwright en CI | 0dfc71e |
| 2026-07-21 | Utilisation joueur | Confort tactile de la vue joueur (tablette). (1) Saisir un pion pour le déplacer n'ouvre plus l'encart d'info : `Inspector.render` ne l'ouvre plus tant qu'un glisser est en cours (`Editor.isPointerDragging`) ; un simple tap sans déplacement l'ouvre toujours (au relâchement, `forceOpen`). (2) Le bouton « ⓘ Infos » est remonté au-dessus des contrôles de zoom (même coin bas-droite) — fini le chevauchement. (3) Le pincement tactile zoome désormais la CARTE et non la page entière : `touch-action: pan-x pan-y` sur `.map-wrapper` (pan un doigt natif conservé, zoom de page du navigateur neutralisé) + gestionnaire de pincement à deux doigts (écoute en capture pour voir les deux doigts malgré le `stopPropagation` des jetons, zoom centré sur le milieu des doigts, calé sur `requestAnimationFrame`, annulation propre de tout geste plateau). (4) Nouveau bouton plein écran dans le header (entrée/sortie via l'API Fullscreen, libellé et `aria-pressed` synchronisés sur `fullscreenchange`, masqué si non supporté). Tests 40/40 unitaires ; intégration Playwright en CI | 0dfc71e |
| 2026-07-21 | Déploiement | Le déploiement GitHub Pages passe de manuel (workflow_dispatch avec saisie « DEPLOY ») à automatique : tout push sur `main` enchaîne tests → build → publication (`deploy-pages.yml`, garde-fou de confirmation retiré ; déclenchement manuel conservé pour republier sans commit). `ci.yml` se limite désormais aux pull requests afin de ne pas relancer la suite Playwright deux fois par push sur `main`. | 2eb16f4 |
| 2026-07-21 | Performance | Plan `docs/perf-plan.md` exécuté (P1→P5), aucun changement visuel attendu. **P1** : fin du layout thrashing du drag — géométrie du plateau (`rect`+`cellPx`) figée au 1er mouvement (`dragGeom`, `MapView.captureBoardGeometry`, `gridPosFromEvent(e, cache)`), rafraîchie sur scroll/resize ; `pointermove` coalescés en une maj/frame (rAF), vidés de force au `pointerup`. **P2** : entités, pions, décors, cabines, points de transition positionnés via `transform: translate3d(var(--tx),var(--ty),0)` (composité GPU, helper `setLayerPos`) au lieu de `left/top` ; `will-change` sur l'élément glissé ; keyframes d'état + hover/selected portent la position ; cases/labels de pièce restent en `left/top`. **P3** : `renderCoverages` mémorise la géométrie des cônes par entité (`coveragePolyCache`, signature = jeton occulteurs + `cellPx` + pos + cap + cotes) ; un cône statique n'est plus ray-casté à chaque frame (`getCoverageCacheStats`). **P4** : au drop d'un pion sans changement d'étage, la vue MJ fait un simple `settleTokenDrag` (pas de `render()` complet), la vue joueur garde le `render()` pour révéler les découvertes ; `updateCameraFeedVisibility` neutralisé pendant un drag actif (resync au relâchement). **P5** : `renderRooms/Decors/Entities/Tokens/Transitions` réconcilient leurs nœuds par clé (`reconcileLayer`, pendant HTML de `reconcileSvgGroup`) — nœuds/écouteurs/images réutilisés, `className`+styles réaffectés par frame, enfants reconstruits sur signature de contenu ; `renderOverlay` réutilise ses groupes SVG ; couche « obstacles » = décors + cabines unifiés. Reste P6 (culling viewport, optionnel). Tests unitaires 40/40 + smoke rendu hors navigateur (réutilisation des nœuds, 0 rebuild de cône statique) ; intégration Playwright en CI, vérifs tablette manuelles à faire | `0d392e1` |
...
| 2026-07-23 | Documentation / validation finale | Revue de l'état du projet. (1) `README.md` et `docs/production-runbook.md` décrivaient encore l'ancien déploiement manuel (`workflow_dispatch` + saisie « DEPLOY »), alors que `2eb16f4` (2026-07-21) l'a rendu automatique sur push `main` — les deux documents sont mis à jour pour refléter le workflow réel. (2) Dernières cases physiques du plan (§7.7 révélation multi-écrans, §12.3/§14/§15.7 gestes tactiles et conflit deux machines) cochées : validées en conditions réelles (tablette + MacBook). Aucun changement de code. | `7931de2` |
| 2026-07-23 | Confort d'édition | Le quadrillage de fond (`--grid-line`, `css/style.css`) reste à son opacité d'origine (0,05) en vue joueur et en prévisualisation MJ — jugée correcte — mais passe à 0,16 en édition MJ, où il était parfois difficile à repérer. Portée via `body:not(.player-mode) { --grid-line: … }` (surcharge de variable CSS, aucun changement de balisage). Tests 47/47 unitaires verts (changement purement visuel, aucune couverture dédiée). | `0c4e44d` |
| 2026-07-23 | Grille du plan | Surface constructible réglable (une seule grille partagée par tous les étages, décision utilisateur — pas de taille par étage). Nouveau `Store.resizeGrid(cols, rows)`/`previewGridResize` (`js/store.js`) : agrandir s'applique toujours ; rétrécir est bloquant et jamais silencieux — `describeGridOverflow` liste, étage par étage, chaque pièce (cases hors bornes), décor, dispositif (+ waypoints de ronde), pion et point de transition (gaine d'ascenseur comprise) qui dépasserait, sans rien recaler ni supprimer. Contrôle MJ dans le panneau Outils (`js/editor.js`, section repliable « Grille du plan » : colonnes/lignes + bouton Appliquer, `alert()` listant les blocages). Bornes 4×4 minimum. 4 tests unitaires (agrandir, rétrécir refusé multi-catégories, rétrécir accepté, gaine d'ascenseur hors bornes) ; vérification manuelle Playwright (refus 5×5 sur le plan par défaut avec liste correcte, agrandissement 30×20 appliqué et rendu). Tests 51/51 unitaires + 19/19 intégration verts | `ac1f790` |
| 2026-07-23 | Outillage MJ | Bouton « ⏻ Autres sessions » dans le header MJ : déconnecte à distance les autres instances MJ ouvertes sur d'autres machines (même compte `ethoril@gmail.com` connecté ailleurs). **Présence** : nouvelle collection `plans/main/sessions/{instanceId}` — chaque instance MJ s'enregistre à la connexion (`Cloud.registerSession`, label lisible navigateur·OS·heure + email), bat le cœur toutes les 30 s (`heartbeatSession`), et retire son document à la déconnexion/fermeture d'onglet (`endSession`, au mieux via `pagehide` ; sinon purge par péremption > 10 min côté clients). `instanceId` = `crypto.randomUUID()` (repli aléatoire). **Déconnexion coopérative** (Firestore n'a pas de push serveur) : le clic marque `kickedAt` sur chaque autre session vivante (`kickSession`) ; chaque instance surveille son propre document via `subscribeSessions` et, voyant son `kickedAt`, appelle `Cloud.logout()` (retour lecture seule + alerte). Le bouton affiche le nombre d'autres sessions vivantes (battement < 90 s), est désactivé à zéro, et masqué en vue joueur (`Store.ui.readOnly`). Confirmation listant les sessions ciblées avant déconnexion. **Règles** : bloc `match /sessions/{sessionId} { allow read, write: if isAdmin(); }` (toutes les instances partagent le compte MJ, donc chacune peut marquer les autres ; jamais lu côté joueur). Câblage réseau/DOM (cloud.js + main.js) non couvert par le harnais vm des tests unitaires : vérifié par revue + build. Tests 51/51 unitaires verts, build OK. **⚠ Déployer les règles** (`firebase deploy --only firestore:rules`) avant usage en prod | `d250db5` |
| 2026-07-23 | Correctif rendu | Les bords des cônes de vision en balayage clignotaient au contact d'un obstacle. Cause : la boucle d'animation (`js/anim.js`) sautait `renderCoverages` dès que la signature du flux caméra changeait (`!feedChanged`), or `updateCameraFeedVisibility` ne redessine pas les cônes (salles/décors/entités/transitions seulement). La signature change pile quand un bord de cône franchit un obstacle → la géométrie du cône figeait une frame puis sautait d'un coup (bords « clignotants »). Régression introduite par `b904595` (flux caméras piratées). Correctif : découpler les deux rendus indépendants — toujours lancer `updateCameraFeedVisibility` **et** `renderCoverages` quand nécessaire, comme avant `b904595`. Le re-rendu reste bon marché (cache `coveragePolyCache` : un cône statique n'est pas ray-casté à nouveau). Boucle d'anim non couverte par le harnais vm : vérifié par revue. Tests 51/51 unitaires + 19/19 intégration verts. | `07f96b1` |
| 2026-07-23 | Correctif synchro | Faux « CONFLIT // MODIFICATION DISTANTE DÉTECTÉE » côté MJ alors qu'aucune autre session MJ n'écrit (seul un joueur passif connecté). Cause : course entre l'écho `onSnapshot` de notre propre écriture (`hasPendingWrites=false`, révision N+1) et la résolution de `runTransaction`. `saveNow` (`js/store.js`) laisse `dirty=true` et `plan.revision` à N pendant toute la sauvegarde (mise à jour seulement dans son `.then`) ; si l'écho serveur arrive avant ce `.then`, `processRemotePlan` (`js/main.js`) voit `dirty` + révision distante N+1 ≠ révision locale N périmée → faux conflit. Correctif : ignorer les échos distants du document plan tant qu'une sauvegarde est en vol (`if (isAdmin && Store.isSaveInFlight()) return;`, MJ uniquement). Sûr car une vraie écriture concurrente pendant notre transaction est déjà détectée par la transaction optimiste (relecture + `revision-conflict`). N'affecte ni les pions ni les découvertes (collections Firestore distinctes, abonnements séparés). Chemin réseau/DOM non couvert par le harnais vm : vérifié par revue ; tests 51/51 unitaires + 19/19 intégration verts (dont conflit de révision et synchro de pion). | `830b92c` |
| 2026-07-23 | Correctif rendu | Vraie cause du clignotement des bords de cône en balayage (le correctif `07f96b1` avait supprimé un saut de frame réel mais connexe, pas cet artefact). `conePolygon` (`js/map.js`) échantillonnait des rayons à des angles **ancrés sur le cap** : quand la caméra balaie, ces rayons glissent sur l'arête d'un obstacle et le rayon le plus proche d'un coin bascule d'une frame à l'autre entre « touche » (court) et « manque » (pleine portée) → un pic surgit/disparaît sur le bord (clignotement, uniquement en balayage ET au contact d'un obstacle — confirmé par l'utilisateur : sans obstacle, pas de clignotement). Correctif : algorithme de polygone de visibilité — en plus de l'échantillonnage régulier, lancer un rayon vers **chaque coin d'occulteur** à portée et dans l'ouverture, encadré de ±EPS (~0,03°), puis trier les rayons par écart angulaire (polygone en étoile, donc simple). Les coins épinglent la silhouette exacte, stable quel que soit le cap ; les rayons réguliers glissent le long du mur sans à-coup. Coût borné aux cônes animés (les statiques restent en cache `coveragePolyCache`) et aux coins réellement dans le champ (filtre portée + ouverture). Nouveau helper `castRay`. Tests 51/51 unitaires + 19/19 intégration verts (dont flux caméra piraté limité au cône). | `06bded6` |
| 2026-07-23 | Outillage build | Indicateur de version dans le header pour lever le doute « suis-je sur la dernière version ou sur du cache ? ». Le build (`build-site.mjs`) écrit désormais l'identifiant du commit (SHA court, via `GITHUB_SHA` en CI ou `git rev-parse` en local) à deux endroits : le `<meta name="app-build">` **embarqué** dans la page servie et un `version.json` à la racine du site. Au chargement, `js/version.js` lit `version.json` **sans cache** (`no-store` + cache-buster) et le compare au build embarqué : identiques → badge « 🟢 v_… » (à jour) ; différents → « ⟳ Nouvelle version — Recharger » cliquable (l'`index.html` affiché vient du cache navigateur ou de GitHub Pages). Re-contrôle au retour sur l'onglet (`visibilitychange`, sans timer) pour couvrir un onglet resté ouvert pendant un déploiement — le cas des joueurs (voir mémoire projet). Build de dev / `file://` → « v_dev » sans contrôle (pas de faux positif). Le cache-busting des assets JS/CSS (hash de contenu, `e33723d`) était déjà en place ; ce qui manquait était la **visibilité de la fraîcheur de la coquille HTML**. `version.js` lui-même tamponné par le hash de contenu comme les autres assets. Tests 51/51 unitaires verts, build OK. | `À renseigner` |
